...
# data path
path = 'feature-extraction-csv-files/'
# datasets 
#datasets = ['Billund_90_10', 'Skive_50_50', 'Skive_Billund_10_90', 'Skive_Billund_50_50']
datasets = ['Skive_50_50']

decompositions = ['ICA', 'NMF']
...
for dataset in datasets:
    for decomp in decompositions:
        try:
            ...
            dataset_ = '{}/{}.csv'.format(dataset, decomp)
            print ('Running classification on {}'.format(dataset_))

            data = np.loadtxt(path + dataset_, delimiter=',', dtype=object, encoding='utf-8')
            
            # split input and target
            X = data[:, 1:-1].astype(float)
            y = data[:,-1].astype(int)
            print('Succesfully loaded {}'.format(dataset))
        ...
        print ('\n\nFinal accuracy results for {}'.format(dataset))
        ...
        result['SVM'][dataset_] = np.mean(SVM_score)
        result['RF'][dataset_] = np.mean(random_forest_score)
